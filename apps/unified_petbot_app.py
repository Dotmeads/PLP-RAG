# -*- coding: utf-8 -*-
"""
Unified PetBot App - Combines RAG System + Azure Pet Search
Integrates pet care Q&A with advanced pet adoption search
"""
...
def bootstrap_azure_components():
    """Load Azure components for pet search"""
    try:
        ...
        with st.spinner("Downloading NER model from Azure Blob..."):
        ...
        if faiss_index is not None:
            st.success(f"FAISS index loaded from {local_mr_dir()}")
        else:
        ...
        # Parse media columns to real lists
        ...
        st.error(f"Failed to load Azure components: {str(e)}")
        st.info("Azure pet search will be disabled. RAG system will still work.")
...
def bootstrap_rag_system():
    """Load RAG system for pet care Q&A"""
    try:
        rag = ProposedRAGManager(collection_name="unified_petbot_documents", use_openai=False)
...
def main():
    ...
    st.caption("Pet Care Q&A + Advanced Pet Search")
    ...
    with st.sidebar:
        ...
        mode = st.radio(
            "Choose Mode",
            ["Pet Care Q&A", "Pet Search", "Both"],
            index=2,
            help="Pet Care Q&A: Ask questions about pet health, feeding, etc.\nPet Search: Find pets for adoption\nBoth: Switch between modes"
        )
        
        st.divider()
        
        # Load components based on mode
        if mode in ["Pet Care Q&A", "Both"]:
    ...
    # Main content area
    if mode == "Pet Care Q&A":
        st.header("🐕 Pet Care Q&A")
        st.caption("Ask questions about pet health, feeding, care, etc.")
        ...
        # Chat input
        if prompt := st.chat_input("Ask about pet care..."):
            # Add user message
    ...
    elif mode == "Pet Search":
        st.header("🔍 Pet Search")
        st.caption("Find pets for adoption with advanced search")
        
        if azure_components[0] is None:
            st.error("Azure pet search not available. Please check the sidebar for errors.")
            return
        ...
        # Process search (simplified version of the original)
        t0 = time.time()
        ...
        with st.expander("🔎 Debug — NER spans & facets"):
            ...
            if prefilter_rows is not None:
                st.write(f"**Pre-filter by breed substring rows**: {prefilter_rows}")
            st.write(f"**Age facet used**: {(age_floor_mo, age_ceil_mo)}")
        ...
        if not pool_ids:
            ...
            # Top-K + facet-preserving re-rank
        ...
        st.header("🐾 Unified PetBot")
        st.caption("Switch between Pet Care Q&A and Pet Search")
        
        # Tab interface
        tab1, tab2 = st.tabs(["🐕 Pet Care Q&A", "🔍 Pet Search"])
        ...
        with tab1:
            ...
            if rag is None or chatbot is None:
                ...
                question = st.text_input("Ask about pet care...", placeholder="What can I feed my dog?")
                if st.button("Ask") and question:
                    with st.spinner("Thinking..."):
                        try:
                            response = chatbot.handle_message(question)
                            st.write(response)
                        except Exception as e:
        ...
        with tab2:
            ...
            if azure_components[0] is None:
                ...
                st.info("Pet search functionality available. Switch to 'Pet Search' mode for full interface.")
